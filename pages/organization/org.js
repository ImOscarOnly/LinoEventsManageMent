$(document).ready(function () {
    ...
    function getAllData() {
      sampleArray = [];
      $.get({
        url: "organization/orgCrud.php",
        data: { getData: "getData" },
        success: function (data) {
          let newData = JSON.parse(data);
          let table = $("#rioMainTable");
          newData.forEach((organizations, i) => {
            sampleArray.push(organizations);
            let tableRow = $("<tr>", { id: newData.id });
            $("<td>", { class: "text-wrap", html: i + 1 }).appendTo(tableRow);
            $("<td>", { class: "text-wrap", html: organizations.organization_name }).appendTo(
              tableRow
            );
            $("<td>", { class: "text-wrap", html: organizations.organization_type }).appendTo(
                tableRow
              );
              $("<td>", { class: "text-wrap", html: organizations.organization_leader }).appendTo(
                tableRow
              );
    ...
    $("#create-new").click(function () {
      $("#rioModalLabel").html("Add organizations");
      $("#rioModal").modal("show");
    ...
    function update(index) {
      $("#rioModal").modal("show");
      $("#rioModalLabel").html("Update organizations");
    ...
    function view(index) {
      $("#rioModal").modal("show");
      $("#rioModalLabel").html("View organizations");
      let models = sampleArray[index];
      Object.keys(models).map((key) => {
